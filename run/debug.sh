gpu=1
run_name='llama'
output_dir='/opt/data/private/wtc_beifen/bird/exp_res/output/debug'
...
# deepspeed --num_gpus=2 train_bird.py --seed 1 --cfg /opt/data/private/wtc_beifen/bird/finetuning/configure/experiment/llama2.cfg \
# --deepspeed ${deep_speed} \
# --run_name ${run_name} --logging_strategy steps --logging_first_step true --logging_steps 4 --evaluation_strategy steps \
# --eval_steps 10 --metric_for_best_model avr --greater_is_better true --save_strategy steps --save_steps 10 \
...
CUDA_VISIBLE_DEVICES=0 python train_bird.py --seed 1 --cfg /opt/data/private/wtc_beifen/bird/finetuning/configure/experiment/llama2.cfg \
--run_name ${run_name} --logging_strategy steps --logging_first_step true --logging_steps 4 --evaluation_strategy steps \
--eval_steps 10 --metric_for_best_model avr --greater_is_better true --save_strategy steps --save_steps 10 \
--save_total_limit 1 --load_best_model_at_end --gradient_accumulation_steps 1 --num_train_epochs 10 \
--learning_rate 5e-5 --do_train false --do_eval true --do_predict false --predict_with_generate true \
